    ...
    <div className="md-overlay" onClick={onClose} role="dialog" aria-modal="true">
      <div className="md-card cal-modal" ref={cardRef} tabIndex={-1} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="md-section">
          ...
          <SubRow label={`All ${matches.length} games`} httpsUrl={FEED} />
        ...
        <div className="md-section">
          ...
          <div className="cal-downloads">
            <button onClick={() => downloadICSCollection(matches, `${LEAGUE.ics.filenameBase}-all.ics`)}>
              All games ({matches.length})
            </button>
            <button onClick={() => downloadICSCollection(filtered, `${LEAGUE.ics.filenameBase}-filtered.ics`)}>
              Current filter ({filtered.length})
            </button>
            {count > 0 && (
              <button onClick={() => downloadICSCollection(myMatches, `${LEAGUE.ics.filenameBase}-my-teams.ics`, `${LEAGUE.edition} — My Teams`)}>
