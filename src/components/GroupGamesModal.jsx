...
// Once a team has a mathematically locked quarter-final opponent, show who they
// are set to play. In the men's format both top-two teams of a second-round group
// reach the quarter-finals, and the opponent is the crossover placing of another
// second-round group, so once both placings are pinned the matchup is fixed.
const ROUND_TITLE = { QF: 'Quarter-final' }
...
function KnockoutSection({ team, knockout }) {
  ...
  return (
    <div className="md-section gg-knockout">
      <h4>{ROUND_TITLE[knockout.round] || 'Knockout'}</h4>
      <p className="gg-ko-sub">
        {team} have reached the quarter-finals and{' '}
...
export default function GroupGamesModal({ group, team, matches, tz, hideScores, knockout, onClose }) {
  ...
  const fixtures = matches
    .filter((m) => (m.stage === 'R1' || m.stage === 'R2') && m.group === group)
