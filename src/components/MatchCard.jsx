...
import { sideNames, stageLabel as gameStageLabel } from '../utils/slots.js'
...
// Tooltip on a second-round team's name in a card: which quarter-final each of the
// group's top two would play. `slot` comes from the slot map, which is keyed by
// SECOND-ROUND group (I-L), so this only fires for a second-round game card; a
// first-round game's group (A-H) is not in the map, so `slot` is undefined and no
// tooltip shows. There is no bye and no qualification round in this format: both of
// a second-round group's top two go straight to the quarter-finals.
function slotTooltip(group, slot) {
...
function Team({ name, ko, clinch, group, slot, feeder }) {
  ...
  const nameTitle = slotTooltip(group, slot) || undefined
  return (
...
export default function MatchCard({ match, tz, hidden = false, clinch, slotMap, byNum }) {
  ...
  const stageLabel = gameStageLabel(match)
    ...
    <article className={`card status-${status}`}>
      ...
      <div className="card-body">
        ...
        <div className="matchup">
          <Team name={side1} ko={match.ko} clinch={match.stage === 'R1' ? clinch?.[match.t1] : undefined} group={match.group} slot={slotMap?.[match.group]} feeder={feederTeams(side1, byNum)} />
          {hasScore ? (
            scoreHidden ? (
              <button
          ...
          <Team name={side2} ko={match.ko} clinch={match.stage === 'R1' ? clinch?.[match.t2] : undefined} group={match.group} slot={slotMap?.[match.group]} feeder={feederTeams(side2, byNum)} />
        </div>
