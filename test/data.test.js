...
// these assert the SHAPE of the tournament against facts that are true of the
// men's 2027 edition and would break loudly if a regeneration went wrong.
//
// Everything team-specific is a PROVISIONAL placeholder draw (the real draw is not
// held until spring 2027), so these assert the format, not real qualifiers.
...
describe('teams', () => {
  ...
  // The ranking stands in for FIBA's drawing of lots (the final tie-break), so it
  // must be distinct and total across all 32, or an unplayed group's order and the
  // projected bracket become non-deterministic. It is a dense 1-32 over the
  // placeholder field, not sparse world ranks.
  it('gives every team a distinct rank from 1 to 32', () => {
    ...
    expect(RANK_BY_TEAM['United States']).toBe(1)
    expect(RANK_BY_TEAM.Qatar).toBe(32)
  })
  ...
  it('lists each group strongest-first by rank, not alphabetically', () => {
    ...
    // Group A is the sharp case: alphabetical put Dominican Republic first.
...
describe('venues and times', () => {
  it('plays every game in the Doha metro, in one timezone', () => {
    for (const v of Object.values(VENUES)) {
      expect(v.country).toBe('Qatar')
      expect(v.tz).toBe('Asia/Qatar')
    }
    expect(Object.keys(VENUES)).toHaveLength(4)
    expect(new Set(Object.values(VENUES).map((v) => v.city))).toEqual(
      new Set(['Lusail', 'Doha', 'Al Rayyan', 'Al Wakrah']),
    )
  })

  it('names the four host arenas', () => {
    expect(VENUES.lusail.name).toBe('Lusail Sports Arena')
    expect(VENUES.attiyah.name).toBe('Ali Bin Hamad Al Attiyah Arena')
  })

  it('stores every game at the Qatar offset, inside the window', () => {
    for (const g of GAMES) {
      expect(g.ko).toMatch(/\+03:00$/)
      const t = new Date(g.ko)
      expect(t >= new Date('2027-08-27T00:00:00+03:00')).toBe(true)
      expect(t <= new Date('2027-09-13T00:00:00+03:00')).toBe(true)
    }
  })
...
describe('the placeholder generated board', () => {
  ...
  it('ships every game unplayed, with no ESPN id and no broadcast yet', () => {
    for (const g of GAMES) {
      expect(g.score, `game ${g.num}`).toBeUndefined()
  ...
  it('carries slot labels and null teams on every non-first-round game', () => {
...
// US broadcast data is not yet set for 2027 (rights unannounced), so every game's
// `tv` is empty. The broadcast.js module is still the women's edition's WBD data,
// carried unchanged; these assert it imports and is well-formed so it stays
// covered, and are the reminder to replace it with the men's rights when known.
describe('US broadcast module (placeholder, pending 2027 rights)', () => {
