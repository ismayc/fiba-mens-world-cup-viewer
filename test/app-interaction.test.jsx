...
describe('filtering', () => {
  ...
  it('searches, and clears every filter at once', async () => {
    ...
    fireEvent.change(screen.getByRole('searchbox'), { target: { value: 'team: Japan' } })
    expect(document.querySelectorAll('.card')).toHaveLength(3)
...
describe('the schedule list', () => {
  ...
  it('hides a completed stage and offers it back', async () => {
    // Every first-round game final: the first round drops out of the Schedule.
    const played = allGroupsPlayed()
    feed(espnScoreboard(played.filter((g) => g.stage === 'R1'), Object.fromEntries(
      played.filter((g) => g.stage === 'R1').map((g) => [g.num, { state: 'post', score: g.score }]),
    )))
    mount()
    await screen.findByText(/games with scores/)
    const note = await screen.findByText(/First round complete/)
    expect(note).toBeInTheDocument()
    fireEvent.click(within(note).getByRole('button', { name: /Show first-round games/ }))
...
describe('the view strip', () => {
  it('appears when the main nav scrolls away, and switches view', async () => {
    ...
    expect(await screen.findAllByText(/second-round groups/)).not.toHaveLength(0)
  })
...
describe('modals', () => {
  ...
  it('jumps to the bracket from a second-round projection', async () => {
    // Every first-round game final so the second round seeds and its "as it
    // stands" projection offers the bracket jump links.
    const played = allGroupsPlayed()
    feed(espnScoreboard(played.filter((g) => g.stage === 'R1'), Object.fromEntries(
      played.filter((g) => g.stage === 'R1').map((g) => [g.num, { state: 'post', score: g.score }]),
    )))
...
describe('result alerts', () => {
  ...
  it('switches alerts back off without asking again', async () => {
    localStorage.setItem('fmwc:scoreAlerts', JSON.stringify({ enabled: true, scope: 'all' }))
    mount()
    await ready()
    ...
    fireEvent.click(box)
    await waitFor(() =>
      expect(JSON.parse(localStorage.getItem('fmwc:scoreAlerts')).enabled).toBe(false),
    )
  })
  ...
  it('restores a saved scope, and falls back for an unreadable one', async () => {
    ...
    localStorage.setItem('fmwc:scoreAlerts', 'not json')
    mount()
    await ready()
...
describe('App remaining arms', () => {
  ...
  it('says "1 first-round game hidden" in the singular', async () => {
    // Complete the first round, then filter so the hidden-stage note counts a
    // known number of first-round games.
    const played = allGroupsPlayed()
    const r1 = played.filter((g) => g.stage === 'R1')
    feed(espnScoreboard(r1, Object.fromEntries(
    ...
    fireEvent.click(document.querySelector('.search-toggle'))
    const noteText = () => document.querySelector('.schedule-note')?.textContent || ''
    // Japan plays three first-round games.
    fireEvent.change(screen.getByRole('searchbox'), { target: { value: 'team: Japan' } })
