...
describe('the results feed', () => {
  it('reports that nothing has been played yet', async () => {
    mount()
    // NOTE: App.jsx still renders the leftover women's date string here
    // ("tip-off is September 4, 2026"); matching a substring keeps this robust to
    // the src fix. See the fork report's list of leftover strings.
...
describe('result alerts', () => {
  ...
  it('persists the preference', async () => {
    ...
    await waitFor(() =>
      expect(JSON.parse(localStorage.getItem('fmwc:scoreAlerts')).enabled).toBe(true),
    )
...
describe('theme', () => {
  it('toggles and remembers the palette under this app’s own key', async () => {
    ...
    expect(localStorage.getItem('fmwc:theme')).toBeTruthy()
