...
// The summary fixture (SUMMARY_401907390) is a real ESPN box-score payload used
// here as an OPAQUE input to exercise the parser and the box-score UI; its teams
// (Japan v Mali) are the fixture's, not this edition's. A standalone game object
// carries them so the parser/orderSides/line-score assertions line up with the
// payload, independent of the men's committed board.
const JAPAN_MALI = {
  num: 1,
  stage: 'R1',
  group: 'A',
...
describe('MatchDetail box score', () => {
  beforeEach(() => {
    ...
    vi.setSystemTime(new Date('2027-08-27T18:00:00Z'))
  })
  afterEach(() => {
    cleanup()
    vi.useRealTimers()
  ...
  it('fetches for a game that has tipped but has no final score yet', async () => {
    global.fetch = ok(synthetic())
    const live = { ...JAPAN_MALI, score: null, ko: '2027-08-27T11:00:00+03:00' }
  ...
  it('does not fetch for a game that has not tipped, or one with no ESPN id', () => {
    global.fetch = ok(SUMMARY_401907390)
    const future = { ...JAPAN_MALI, score: null, ko: '2027-09-08T18:00:00+03:00' }
